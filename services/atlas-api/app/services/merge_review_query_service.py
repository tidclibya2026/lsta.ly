"""Query and controlled review operations for merge proposals.

This module provides listing, summaries, assignment, comparison, history,
preview, and decision recording. It does not execute a physical merge or
promote records into the authoritative registry.
"""
...
def _proposal_stmt():
    return select(MergeProposal).options(
        selectinload(MergeProposal.decisions)
    )


def get_merge_proposal(session: Session, proposal_id):
    statement = _proposal_stmt().where(
        MergeProposal.id == proposal_id
    )
    proposal = session.scalar(statement)

    if proposal is None:
        raise LookupError("merge proposal not found")
